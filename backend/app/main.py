...
app = FastAPI(title='Qarar AI — Decision Intelligence Platform', version='10.0.0-beta.1', lifespan=lifespan)
...
@app.get('/')
def root():
    return {'name': 'Qarar AI V10', 'category': 'Enterprise Decision Intelligence Platform', 'docs': '/docs',
            'mcp': settings.mcp_public_base_url.rstrip('/') + '/mcp', 'security': 'authenticated tenant-scoped API'}


@app.get('/api/health')
def health():
    return {'status': 'ok', 'version': '10.0.0-beta.1', 'ai_enabled': settings.ai_enabled, 'provider': settings.ai_provider,
